...
describe('warmup-then-parallel integration', () => {
  it('buildChunks produces first two warmup chunks smaller than later chunks', () => {
    // Each block: 200 tokens. WARMUP=200 → 1 block fills warmup.
    // TARGET=600 → 3 blocks per normal chunk.
  ...
  it('buildChunks with many blocks produces warmup chunks then normal chunks', () => {
    // 200 token blocks → 1 block per warmup chunk
    ...
    // First two chunks: 1 block each (warmup limit)
    expect(chunks[0].blocks.length).toBeLessThanOrEqual(1);
    expect(chunks[1].blocks.length).toBeLessThanOrEqual(1);
  });
  ...
  it('only first two chunks are warmup sized with small blocks', () => {
    ...
    // WARMUP=200 → 4 blocks of 50 = 200 tokens
    // Chunk 1 and 2 warmup: each ≤200 tokens (≤ 4 blocks)
    expect(chunks[0].blocks.length).toBeLessThanOrEqual(4);
    expect(chunks[1].blocks.length).toBeLessThanOrEqual(4);
    // Chunk 3+ normal: can hold more (TARGET=600 → up to 12 blocks)
    for (let i = 2; i < chunks.length; i++) {
      expect(chunks[i].estimatedTokens).toBeLessThanOrEqual(620);
    }
